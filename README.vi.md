...
Đây là bộ workflow mở, dạng module cho ChatGPT, Gemini, Claude và Google Flow. Bạn không cần API, không cần server chứa ảnh, cũng không phải đổi tên file theo quy tắc rườm rà. Các giao diện hội thoại có thể dùng prompt Instant Run hoặc cài Custom GPT, Gem hay Claude Project với [bốn Knowledge bundle sẵn sàng upload](bundles/README.vi.md). Google Flow dùng package Tool Builder tự chứa riêng.
...
- **Reference Style Profile** được trích xuất động từ chính hình tham khảo người dùng cung cấp.
- **Style Card** là template dự phòng khi không có hình tham khảo.
...
Ảnh chụp sản phẩm gốc luôn là nguồn có thẩm quyền về sản phẩm. Hình tham khảo chỉ được cung cấp bề mặt nền, bảng màu, hướng và độ mềm của ánh sáng, Bóng tiếp xúc, mood và khoảng trống tương thích. Tuyệt đối không lấy sản phẩm, đạo cụ, người, bao bì, chữ, logo hay watermark từ hình tham khảo. Khi có hình tham khảo, workflow tạo Reference Style Profile động trực tiếp từ hình đó; không đổi tên nó thành Style Card gần nhất và không để preset làm lệch kết quả.
...
Với ChatGPT, Gemini hoặc Claude, quy trình hội thoại cơ bản là:

1. Chọn ChatGPT, Gemini hoặc Claude. Nếu dùng Google Flow, làm theo [hướng dẫn Tool Builder tiếng Việt](platforms/google-flow/setup.vi.md).
...
| Google Flow | [Builder prompt tự chứa](platforms/google-flow/builder-prompt.md) | [Cài bằng Tool Builder](platforms/google-flow/setup.vi.md) | Dựng Tool tái sử dụng với Single ổn định và `BATCH EXPERIMENTAL` cho 2–20 job sản phẩm tách biệt; phải kiểm chứng runtime, credits, download và chất lượng hình ảnh. |

Hãy đọc `limitations.md` của từng package trước khi đưa vào sản xuất. Google Flow không dùng bốn Knowledge bundle dành cho hội thoại; Tool Builder trả lời rằng đã làm xong chưa chứng minh runtime thực sự có mọi chức năng. Phiên bản 1 không tự động xuất prompt hoặc chuyển người dùng từ Claude sang nền tảng khác.
...
Khi có hình tham khảo, Reference Style Profile động từ hình đó là nguồn style. Các Style Card dưới đây chỉ là lựa chọn dự phòng khi không có reference, hoặc khi người dùng chủ động yêu cầu một card cụ thể. Style Card không bao giờ cho phép thay đổi sản phẩm.
...
1. **Hiệu chỉnh quy trình.** Chọn một số sản phẩm khó, một hình tham khảo đã duyệt (hoặc một Style Card dự phòng) và một output. Bắt đầu bằng Safe Run.
...
Thay đổi trong core ảnh hưởng đến mọi platform package nên cần review nghiêm ngặt hơn việc thêm Style Card. Ghost mannequin, on-model và Prompt Exporter do người dùng chủ động kích hoạt là roadmap riêng, không nằm ngầm trong Version 1.
