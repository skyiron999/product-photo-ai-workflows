...
5. Thêm các file Markdown sau vào **Knowledge**:
   - toàn bộ file trong `core/`;
   - các module cần dùng trong `products/`;
   - các style trong `styles/`;
   - hai profile trong `outputs/`.
...
Khi repo có phiên bản mới, thay các Knowledge file đã đổi. Nếu `installed-instructions.md` thay đổi, cập nhật lại phần Instructions. Sau mỗi lần cập nhật, test một ảnh trước khi tiếp tục batch sản xuất.
