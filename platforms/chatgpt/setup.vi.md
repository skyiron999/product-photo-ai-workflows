...
Khi đã có hình tham khảo, kiểm tra lock sheet phải ghi `Style source: REFERENCE IMAGE` và `Style Card: NONE — reference-driven`. Style Card có tên chỉ là lựa chọn dự phòng khi không có reference hoặc khi bạn chủ động yêu cầu.

Muốn bám sát nền mẫu tối đa, gửi `SAFE RUN STRICT MATCH ECOMMERCE`. Trước khi tiếp tục, kiểm tra ChatGPT đã ghi match target, vùng nền phải tái tạo và `Pixel-exact guarantee: NO — generative visual match`.
...
5. Thêm đúng bốn file đã gom sau vào **Knowledge**:
   ...
   Bundle Style Card chỉ là thư viện dự phòng, không ghi đè hình tham khảo đã upload. Không upload đồng thời bundle và các file nguồn riêng lẻ vì nội dung sẽ bị trùng.
